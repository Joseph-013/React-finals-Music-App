...
const Layout = ({ playing }) => {
  ...
  return (
    <div className="flex flex-row h-full">
      ...
      <section
        className={
          (navBarToggle ? `pl-16 md:pl-60` : `pl-16`) +
          ` flex-1 flex flex-col pt-16 h-screen w-screen pb-20 bg-[#121C21] overflow-x-hidden`
        }
      >
      ...
      <section className="h-20 fixed bottom-0 overflow-hidden w-full z-player bg-[#084868]">
        <PlayerDocked navBarState={navBarToggle} playing={playing} />
      </section>
    </div>
  );
};
