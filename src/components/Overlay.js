...
function Overlay({ children, overlay, setOverlay, setPlaylists }) {
  ...
  const handleSubmit = (e) => {
    ...
    let newPlaylistName = document.getElementById("newPlaylistNameField").value;
    ...
    return (
      <div className="flex flex-col space-y-6 w-full max-w-72">
        <div className="flex justify-between items-center">
          <span className="text-left text-lg md:text-xl font-bold">
            Create Playlist
          </span>
          <button
            className="bg-cyan-900 hover:bg-cyan-800 p-2 rounded-lg"
        ...
        <form
          ...
          className="relative w-full font-poppins flex justify-start h-10"
        >
          <input
            type="text"
  ...
  return (
    ...
    <div
      className={`absolute border-none top-0 left-0 flex items-center justify-center border w-full h-full z-[10000] overflow-hidden ${
        overlay ? "flex" : "hidden"
      }`}
    >
      <div
